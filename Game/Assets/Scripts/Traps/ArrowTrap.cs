...
public class ArrowTrap : MonoBehaviour
{
    ...
    {
        // the amount of times arrows get shot
        for(float a = 0; a < 3; a++)
        {
            ...
            foreach (GameObject trap in traps)
            {
                ...
                for (int i = 0; i < amount; i++)
                {
                    if (trap.transform.eulerAngles.z == 90 || trap.transform.eulerAngles.z == -90)
                        arrowPos += new Vector3(0, arrow.transform.localScale.y - 0.5f, 0);

                    else arrowPos += new Vector3(arrow.transform.localScale.x - 0.5f, 0, 0);
